...
$query_Get_Paymentpicture = "SELECT * FROM barangay_payment_settings_tbl WHERE barangay_ID = '$barangay_ID' AND doc_type = 'Cedula' ";
$result_Get_PaymentPicture = mysqli_query($conn,$query_Get_Paymentpicture);
$price = "";
if(mysqli_num_rows($result_Get_PaymentPicture) > 0){
    $get_price = "SELECT * FROM barangay_document_types_tbl WHERE barangay_ID = '$barangay_ID' AND doc_Type = 'Cedula'";
