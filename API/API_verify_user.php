...
if(isset($_POST['submit'])){
    ...
    if(mysqli_num_rows($result_Get_info) > 0){
        ...
        if ($check !== false) {
            ...
            echo '<script>alert("Processing Error."); window.location.href="../user_verification.php";</script>';
            $uploadOk = 0;
        }
        #2
        ...
        // END_PROCESS
        if($uploadOk == 0){
            echo '<script>alert("Error Uploading File/s."); window.location.href="../user_verification.php";</script>';
        }
        ...
        $query_Insert_registration = "INSERT INTO barangay_verification_tbl SET process_ID = '$process_ID', requestor_ID = '$requestor_ID', barangay_ID = '$barangayID', requestor_Res_Stat = '$requestor_Res_status', 
        ...
        if(!$result_Insert_registration || !$result_Update_status){
            echo '<script>alert("User Verification Failed."); window.location.href="../user_verification.php";</script>';
        }else{
            ...
            move_uploaded_file($_FILES["txt_Self_Portrait"]["tmp_name"],  "../".$requestor_Image)){
                 ...
                 echo '<script>alert("User Verification Request Submitted."); window.location.href="../user_profile.php";</script>';
            }
        }
    }
}else{
